...
function generate_png {
    ...
    if [ -z "$SOURCE" ];
    then
        SOURCE="$PWD/generated/favicon/$IMAGE_NAME-256.png"
    fi
    ...
    $CONVERT_CMD "$SOURCE" -resize ${WIDTH}x${HEIGHT}! -crop ${WIDTH}x${HEIGHT}+0+0 -alpha On "$PWD/generated/favicon/$IMAGE_NAME-${SIZE}.png"
    $OPTIMIZER_CMD $OPTIMIZER_OPTS "$PWD/generated/favicon/$IMAGE_NAME-${SIZE}.png"
}
